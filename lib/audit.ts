...
export async function writeAudit(entry: { action: string; actorRole: "participant" | "admin" | "system"; actorId?: string; registrationId?: string; paymentProofId?: string; metadata?: unknown }) {
  const { error } = await getAdminClient().from("audit_logs").insert({
    action: entry.action,
    actor_role: entry.actorRole,
    actor_id: entry.actorId ?? null,
    registration_id: entry.registrationId ?? null,
    payment_proof_id: entry.paymentProofId ?? null,
